...
- `assets/css/style.css` — all styling, structured as CSS custom properties (design tokens) declared once on `:root`. The site is intentionally single-theme: white background, black ink, no dark mode — do not reintroduce a `prefers-color-scheme: dark` override or a `data-theme` toggle. Components reference tokens (`--paper`, `--ink`, `--muted`, `--pigment`, `--line`, etc.) rather than literal colors — keep new styles consistent with that pattern.
